...
class fpErrorNotifierHandler
{
  ...
  {
    // Do not initialize twice, please
    if ($this->isInit)
    {
      return;
    }
    ...
    // We want to do all the *magic* only when it will actually matter
    if ($configs['class'] !== 'fpErrorNotifierDriverNull' && php_sapi_name() !== 'cli')
    {
      // Register error handler it will process the most part of errors (but not all)
      set_error_handler(array($this, 'handleError'));

      // Register shutdown handler it will process the rest of the errors
  ...
  public function handleException(Exception $e)
  {
    ...
    while ($previous = $e->getPrevious())
    {
      $message->addSection(
        ...
        $this->notifier()->helper()->formatException($previous)
      );
